...
void main() {
  ...
  testWidgets('actions on unregistered field throws StateError', (
    ...
    expect(
      () => key.currentState?.validate("hello", markSubmitted: true),
      throwsStateError,
    );
    expect(() => key.currentState?.reset(name: "hello"), throwsStateError);
    expect(
      () => key.currentState?.updateFieldData(
    ...
    expect(
      () => key.currentState?.updateFieldRules("hello", []),
      throwsStateError,
    );
  });
  ...
  testWidgets('rogue text field does not crash', (WidgetTester tester) async {
    await tester.pumpWidget(
      boilerplate(child: TextSuperFormField(name: "hello_there")),
    );
